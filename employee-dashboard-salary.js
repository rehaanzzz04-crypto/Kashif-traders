...
(()=>{
 ...
 function init(){
  ...
  function setHero(){const hero=document.querySelector('.hero');if(!hero)return;const h=hero.querySelector('h1'),p=hero.querySelector('p');if(h)h.textContent=user.full_name||user.employee_code||'Employee';if(p)p.textContent=(user.designation||'Employee').replace(/_/g,' ').replace(/\b\w/g,c=>c.toUpperCase())+(user.employee_code?' · '+user.employee_code:'');}
  async function render(){
   ...
   try{
    const r=await fetch('/api/salaries',{cache:'no-store'});if(!r.ok)return;const d=await r.json();const p=(d.salary_profiles||[])[0];if(!p)return;
    const now=new Date(),month=now.getFullYear()+'-'+String(now.getMonth()+1).padStart(2,'0'),rows=(d.records||[]).filter(x=>ym(x.salary_month||x.requested_at)===month);
    ...
    const first=dash.firstElementChild;first?dash.insertBefore(box,first):dash.appendChild(box);box.querySelector('#mySalaryView').onclick=()=>document.querySelector('#nav [data-view="salary-advances"]')?.click();
  ...
  setTimeout(()=>{setHero();render()},250);document.querySelector('#nav [data-view="dashboard"]')?.addEventListener('click',()=>setTimeout(()=>{setHero();render()},400),true);return true;
 }
 let tries=0;const timer=setInterval(()=>{tries++;if(init()||tries>50)clearInterval(timer)},100);
