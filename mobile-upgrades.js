...
(function(){
  ...
  function enhanceSettings(){
    ...
    panel.querySelector('[data-app-action="local"]').onclick=()=>download('http://192.168.0.102:8787','Local ERP');
    panel.querySelector('[data-app-action="check"]').onclick=()=>{const current=String(window.KT_NATIVE_APP_VERSION||VERSION),url=window.KT_ANDROID_APK_URL||'https://github.com/rehaanzzz04-crypto/Kashif-traders/releases/download/v1.0.5/Kashif-Traders-ERP-v1.0.5.apk';if(current===VERSION)notify('Kashif Traders Android app is up to date: v'+VERSION+'.');else if(window.KT_NATIVE_APP_VERSION){notify('Update available: v'+current+' → v'+VERSION+'.');setTimeout(()=>{if(typeof window.KT_START_NATIVE_UPDATE==='function')window.KT_START_NATIVE_UPDATE(url);else download(url,'Android update');},450);}else notify('Latest Kashif Traders app version is v'+VERSION+'.');};
  }
