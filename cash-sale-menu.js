...
(()=>{
  ...
  fetch('/api/auth?action=me',{cache:'no-store'}).then(r=>r.json()).then(j=>{const role=String(j?.user?.designation||'').toLowerCase();const section=document.createElement('div');section.className='section';section.textContent='Sales';nav.insertBefore(section,inventory);if(role==='cashier'){add('Cashier Billing','₨','/cashier-sales.html');add('E-Commerce','EC','/ecommerce-dashboard.html')}else if(role==='salesman')add('Cash Sale','$','/cash-sale.html');else{add('Cash Sale','$','/cash-sale.html');add('Cashier Billing','₨','/cashier-sales.html');add('E-Commerce','EC','/ecommerce-dashboard.html')}}).catch(()=>{});
})();
