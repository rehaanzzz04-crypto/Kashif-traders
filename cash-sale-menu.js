...
(()=>{
  ...
  const installSales=role=>{if(nav.querySelector('[data-kt-sales-section]'))return;const section=document.createElement('div');section.className='section';section.dataset.ktSalesSection='1';section.textContent='Sales';nav.insertBefore(section,inventory);if(role==='cashier'){add('Cashier Billing','₨','/cashier-sales.html');add('E-Commerce','EC','/ecommerce-dashboard.html')}else if(role==='salesman')add('Cash Sale','
})();
