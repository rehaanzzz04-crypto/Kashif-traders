...
(()=>{
 ...
 async function openDashboard(){
  ...
  try{
   ...
   ov.innerHTML=(latest?'<div class="overview-alert"><b>Latest pending approval</b><span>'+esc(latest.who)+' · '+esc(latest.title)+'</span></div>':'')+'<div class="overview-head"><div><b>Assigned access</b><small>Admin workspace permissions</small></div><strong>'+allowed.length+'</strong></div><div class="access-grid">'+allowed.map(x=>'<div class="access-card"><span class="access-dot">✓</span><span>'+esc(x)+'</span></div>').join('')+'</div>';
