...
async function openEcommerce(){
  ...
  $('workspaceTitle').textContent='E-commerce Store Builder';
  $('workspaceSubtitle').textContent=model.company.name+' · Website & Orders';
  $('addRecord').classList.add('hidden');
  $('workspaceBody').innerHTML='<div class="card"><div class="emptyLines">Loading store builder…</div></div>';
  const [dash,productsData,ordersData,categoriesData,mediaData]=await Promise.all([
    ...
    json('/api/bizora-company?action=ecommerce_categories'),
    json('/api/bizora-company?action=ecommerce_media')
  ]);
  let settings=dash.settings||{},products=productsData.records||[],orders=ordersData.records||[],categories=categoriesData.records||[],media=mediaData.records||[];
  let section='overview',editingProduct=null,editingCategory=null,editingMedia=null;
  const storeUrl='/store.html?company='+encodeURIComponent(model.company.code);
  const navItems=[['overview','Overview'],['theme','Theme & Style'],['header','Header'],['home','Home Page'],['media','Media Center'],['categories','Categories'],['products','Products'],['orders','Orders'],['contact','Contact & Delivery'],['seo','SEO & Social'],['publish','Preview & Publish']];
  const themeCards=[['modern','Modern','Clean cards and modern gradients.'],['elegant','Elegant','Refined editorial presentation.'],['minimal','Minimal','Whitespace-first simple storefront.'],['premium','Premium','Dark luxury product-focused design.']];
  ...
  const compressStoreImage=async file=>{
    ...
    canvas.getContext('2d',{alpha:false}).drawImage(img,0,0,canvas.width,canvas.height);img.close?.();
    const blob=await new Promise((resolve,reject)=>canvas.toBlob(b=>b?resolve(b):reject(new Error('Image compression failed')),'image/webp',.8));
    return new File([blob],(file.name||'image').replace(/\.[^.]+$/,'')+'.webp',{type:'image/webp'});
  };
  const compressStoreVideo=async file=>{
    if(!/^video\//.test(file.type)||file.size<=2600000)return file;
    if(!window.MediaRecorder)return file;
    const url=URL.createObjectURL(file),video=document.createElement('video');
    video.src=url;video.muted=true;video.playsInline=true;
    await new Promise((resolve,reject)=>{video.onloadedmetadata=resolve;video.onerror=()=>reject(new Error('Video could not be opened'))});
    if(!Number.isFinite(video.duration)||video.duration>60){URL.revokeObjectURL(url);throw new Error('Video must be 60 seconds or shorter')};
    const max=1280,scale=Math.min(1,max/Math.max(video.videoWidth||1,video.videoHeight||1)),canvas=document.createElement('canvas');
    canvas.width=Math.max(2,Math.round((video.videoWidth||640)*scale));canvas.height=Math.max(2,Math.round((video.videoHeight||360)*scale));
    const ctx=canvas.getContext('2d'),canvasStream=canvas.captureStream?canvas.captureStream(24):null;
    if(!canvasStream){URL.revokeObjectURL(url);return file}
    const mime=['video/webm;codecs=vp9','video/webm;codecs=vp8','video/webm'].find(x=>MediaRecorder.isTypeSupported?.(x))||'video/webm';
    const recorder=new MediaRecorder(canvasStream,{mimeType:mime,videoBitsPerSecond:900000}),parts=[];
    recorder.ondataavailable=e=>{if(e.data?.size)parts.push(e.data)};
    const done=new Promise((resolve,reject)=>{recorder.onstop=()=>resolve(new Blob(parts,{type:mime}));recorder.onerror=()=>reject(new Error('Video compression failed'))});
    let drawing=true;const draw=()=>{if(!drawing)return;ctx.drawImage(video,0,0,canvas.width,canvas.height);requestAnimationFrame(draw)};
    recorder.start(500);draw();await video.play();await new Promise(resolve=>{video.onended=resolve});drawing=false;recorder.stop();
    const blob=await done;URL.revokeObjectURL(url);
    return new File([blob],(file.name||'video').replace(/\.[^.]+$/,'')+'.webm',{type:mime.split(';')[0]});
  };
  const uploadStoreAsset=async(file,statusEl)=>{
    if(!file)return'';
    if(statusEl)statusEl.textContent='Compressing…';
    let out=file;
    if(/^image\//.test(file.type))out=await compressStoreImage(file);
    else if(/^video\//.test(file.type))out=await compressStoreVideo(file);
    else throw new Error('Only image or video files are supported');
    if(out.size>3000000)throw new Error('Compressed media is still over 3 MB. Choose a shorter/smaller file.');
    if(statusEl)statusEl.textContent='Uploading…';
    const dataUrl=await blobToDataUrl(out);
    const result=await json('/api/bizora-asset',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({file_name:out.name,mime_type:out.type,base64:dataUrl})});
    if(statusEl)statusEl.textContent='Uploaded';
    return result.asset.url;
  };
  const wireAssetUploads=root=>{
    (root||document).querySelectorAll('.storeAssetInput').forEach(input=>input.onchange=async()=>{
      const target=(root||document).querySelector(input.dataset.target),status=input.parentElement.querySelector('.assetStatus');
      try{input.disabled=true;const url=await uploadStoreAsset(input.files?.[0],status);if(target&&url){target.value=url;target.dispatchEvent(new Event('change',{bubbles:true}))}}
      catch(e){if(status)status.textContent='';alert(e.message)}finally{input.disabled=false;input.value=''}
    });
  };

  const saveSettings=async patch=>{
    const payload={...settings,...patch,action:'save_ecommerce_settings'};
    const out=await json('/api/bizora-company',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});
    settings=out.settings||payload;messageBox('Store settings saved');
  };

  const refreshData=async()=>{
    const [d,p,o,c,m]=await Promise.all([
      json('/api/bizora-company?action=ecommerce_dashboard'),
      json('/api/bizora-company?action=ecommerce_products'),
      json('/api/bizora-company?action=ecommerce_orders'),
      json('/api/bizora-company?action=ecommerce_categories'),
    ...
    settings=d.settings||settings;Object.assign(dash,d);products=p.records||[];orders=o.records||[];categories=c.records||[];media=m.records||[];
  };

  const orderTable=rows=>'<div class="tablewrap"><table><thead><tr><th>Order</th><th>Customer</th><th>Total</th><th>Payment</th><th>Status</th><th></th></tr></thead><tbody>'+(rows.length?rows.map(o=>'<tr><td><b>'+esc(o.order_number)+'</b><small>'+esc(new Date(o.created_at).toLocaleString('en-GB'))+'</small></td><td>'+esc(o.customer_name)+'<small>'+esc(o.phone)+'</small></td><td>'+money(o.total)+'</td><td>'+esc(o.payment_method||'')+'</td><td><select class="ecomOrderStatus" data-id="'+o.id+'">'+['pending','confirmed','packed','shipped','completed','cancelled'].map(x=>'<option value="'+x+'" '+(o.status===x?'selected':'')+'>'+x+'</option>').join('')+'</select></td><td><button class="secondary ecomOrderView" data-id="'+o.id+'">View</button></td></tr>').join(''):'<tr><td colspan="6">No orders yet</td></tr>')+'</tbody></table></div>';
  const productTable=rows=>'<div class="tablewrap"><table><thead><tr><th>Product</th><th>Category</th><th>Price</th><th>Stock</th><th>Actions</th></tr></thead><tbody>'+(rows.length?rows.map(x=>'<tr><td><b>'+esc(x.product_name)+'</b><small>'+esc(x.sku)+(x.featured?' · Featured':'')+'</small></td><td>'+esc(x.category_name||'—')+'</td><td>'+money(x.price)+'</td><td>'+esc(x.stock_qty)+'</td><td><div class="rowactions"><button class="secondary productEdit" data-id="'+x.id+'">Edit</button><button class="secondary productToggle" data-id="'+x.id+'" data-active="'+(x.active?'1':'0')+'">'+(x.active?'Disable':'Enable')+'</button></div></td></tr>').join(''):'<tr><td colspan="5">No products yet</td></tr>')+'</tbody></table></div>';
  const categoryTable=rows=>'<div class="tablewrap"><table><thead><tr><th>Category</th><th>Slug</th><th>Status</th><th></th></tr></thead><tbody>'+(rows.length?rows.map(x=>'<tr><td><b>'+esc(x.category_name)+'</b></td><td>'+esc(x.slug)+'</td><td><span class="pill '+(x.active?'active':'inactive')+'">'+(x.active?'Active':'Inactive')+'</span></td><td><div class="rowactions"><button class="secondary categoryEdit" data-id="'+x.id+'">Edit</button><button class="dangerAction categoryDelete" data-id="'+x.id+'">Delete</button></div></td></tr>').join(''):'<tr><td colspan="4">No categories yet</td></tr>')+'</tbody></table></div>';
  const mediaTable=rows=>'<div class="tablewrap"><table><thead><tr><th>Media</th><th>Placement</th><th>Status</th><th></th></tr></thead><tbody>'+(rows.length?rows.map(x=>'<tr><td><b>'+esc(x.title||x.media_type)+'</b><small>'+esc(x.media_url)+'</small></td><td>'+esc(x.placement)+'</td><td><span class="pill '+(x.active?'active':'inactive')+'">'+(x.active?'Active':'Inactive')+'</span></td><td><div class="rowactions"><button class="secondary mediaEdit" data-id="'+x.id+'">Edit</button><button class="dangerAction mediaDelete" data-id="'+x.id+'">Delete</button></div></td></tr>').join(''):'<tr><td colspan="4">No media yet</td></tr>')+'</tbody></table></div>';
  ...
  const wireOrders=()=>{
    ...
    document.querySelectorAll('.ecomOrderStatus').forEach(s=>s.onchange=async()=>{await json('/api/bizora-company',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'set_ecommerce_order_status',order_id:Number(s.dataset.id),status:s.value})});await refreshData();renderPanel()});
  };
  const wireProducts=()=>{
    wireAssetUploads($('productForm'));
    document.querySelectorAll('.productEdit').forEach(b=>b.onclick=()=>{editingProduct=products.find(x=>Number(x.id)===Number(b.dataset.id));renderPanel()});
    document.querySelectorAll('.productToggle').forEach(b=>b.onclick=async()=>{await json('/api/bizora-company',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'set_ecommerce_product_status',product_id:Number(b.dataset.id),active:b.dataset.active!=='1'})});await refreshData();renderPanel()});
    if($('cancelProductEdit'))$('cancelProductEdit').onclick=()=>{editingProduct=null;renderPanel()};
    $('productForm').onsubmit=async e=>{e.preventDefault();const f=e.currentTarget,d=Object.fromEntries(new FormData(f));d.action='save_ecommerce_product';d.product_id=editingProduct?.id||null;d.featured=f.elements.featured.checked;d.images=[d.image_url,d.image_2,d.image_3].filter(Boolean);delete d.image_2;delete d.image_3;await json('/api/bizora-company',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(d)});editingProduct=null;await refreshData();renderPanel()};
  };
  const wireCategories=()=>{
    wireAssetUploads($('categoryForm'));
    document.querySelectorAll('.categoryEdit').forEach(b=>b.onclick=()=>{editingCategory=categories.find(x=>Number(x.id)===Number(b.dataset.id));renderPanel()});
    document.querySelectorAll('.categoryDelete').forEach(b=>b.onclick=async()=>{if(!confirm('Delete this category? Products category-less ho jayenge.'))return;await json('/api/bizora-company',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'delete_ecommerce_category',category_id:Number(b.dataset.id)})});await refreshData();renderPanel()});
    if($('cancelCategoryEdit'))$('cancelCategoryEdit').onclick=()=>{editingCategory=null;renderPanel()};
    $('categoryForm').onsubmit=async e=>{e.preventDefault();const f=e.currentTarget,d=Object.fromEntries(new FormData(f));d.action='save_ecommerce_category';d.category_id=editingCategory?.id||null;d.active=f.elements.active.checked;await json('/api/bizora-company',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(d)});editingCategory=null;await refreshData();renderPanel()};
  };
  const wireMedia=()=>{
    wireAssetUploads($('mediaForm'));
    document.querySelectorAll('.mediaEdit').forEach(b=>b.onclick=()=>{editingMedia=media.find(x=>Number(x.id)===Number(b.dataset.id));renderPanel()});
    document.querySelectorAll('.mediaDelete').forEach(b=>b.onclick=async()=>{if(!confirm('Delete this media item?'))return;await json('/api/bizora-company',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'delete_ecommerce_media',media_id:Number(b.dataset.id)})});await refreshData();renderPanel()});
    if($('cancelMediaEdit'))$('cancelMediaEdit').onclick=()=>{editingMedia=null;renderPanel()};
    $('mediaForm').onsubmit=async e=>{e.preventDefault();const f=e.currentTarget,d=Object.fromEntries(new FormData(f));d.action='save_ecommerce_media';d.media_id=editingMedia?.id||null;d.active=f.elements.active.checked;await json('/api/bizora-company',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(d)});editingMedia=null;await refreshData();renderPanel()};
  };

  const renderPanel=()=>{
    const p=$('storeBuilderPanel'),st=dash.stats||{};
    if(section==='overview'){p.innerHTML='<div class="ecomKpis">'+[['Products',st.products],['Active Products',st.active_products],['Pending Orders',st.pending_orders],['Completed Orders',st.completed_orders],['Completed Sales',money(st.completed_sales)]].map(x=>'<div><small>'+esc(x[0])+'</small><b>'+esc(x[1]??0)+'</b></div>').join('')+'</div><div class="card"><div class="reportSectionTitle"><b>Recent Orders</b><span>'+esc((dash.recent_orders||[]).length)+' latest</span></div>'+orderTable(dash.recent_orders||[])+'</div>';wireOrders();return}
    if(section==='theme'){p.innerHTML='<div class="card storeBuilderCard"><div class="builderSectionHead"><div><span class="capEyebrow">4 STORE THEMES</span><h3>Choose Website Style</h3></div></div><div class="themePicker">'+themeCards.map(([id,name,desc])=>'<button type="button" class="themeCard '+((settings.theme_code||'modern')===id?'selected':'')+'" data-theme="'+id+'"><span class="themePreview '+id+'"><i></i><i></i><i></i></span><b>'+name+'</b><small>'+desc+'</small></button>').join('')+'</div><form id="themeForm" class="builderForm"><div class="formGrid"><label>Primary Color<input name="primary_color" type="color" value="'+esc(settings.primary_color||'#176fe8')+'"></label><label>Secondary Color<input name="secondary_color" type="color" value="'+esc(settings.secondary_color||'#7042e8')+'"></label><label>Accent Color<input name="accent_color" type="color" value="'+esc(settings.accent_color||'#16b8c8')+'"></label><label>Page Background<input name="background_color" type="color" value="'+esc(settings.background_color||'#f6f8fc')+'"></label></div><div class="masterActions"><button class="primary">Save Theme</button></div></form></div>';document.querySelectorAll('.themeCard').forEach(b=>b.onclick=()=>{settings.theme_code=b.dataset.theme;document.querySelectorAll('.themeCard').forEach(x=>x.classList.toggle('selected',x===b))});$('themeForm').onsubmit=async e=>{e.preventDefault();await saveSettings({...Object.fromEntries(new FormData(e.currentTarget)),theme_code:settings.theme_code||'modern'});render()};return}
    if(section==='header'){p.innerHTML='<form id="headerForm" class="card builderForm"><div class="builderSectionHead"><div><span class="capEyebrow">COMPANY BRANDING</span><h3>Header</h3><p>Public header par Bizora branding nahi hogi.</p></div></div><div class="formGrid"><label>Store Name<input name="store_name" value="'+esc(settings.store_name||model.company.name)+'" required></label><label>Header Layout<select name="header_layout">'+[['logo_name','Logo + Company Name'],['name_only','Company Name Only'],['centered','Centered Brand']].map(x=>'<option value="'+x[0]+'" '+((settings.header_layout||'logo_name')===x[0]?'selected':'')+'>'+x[1]+'</option>').join('')+'</select></label><label class="span2">Store Logo URL<input name="store_logo_url" value="'+esc(settings.store_logo_url||'')+'"><span class="assetUploadRow"><input class="storeAssetInput" data-target="input[name=store_logo_url]" type="file" accept="image/*"><small class="assetStatus">Upload logo</small></span></label><label class="span2">Announcement Text<input name="announcement_text" maxlength="180" value="'+esc(settings.announcement_text||'')+'" placeholder="Free delivery, Ramadan offer, new arrivals..."></label></div><div class="builderToggleGrid"><label><input name="show_search" type="checkbox" '+(settings.show_search!==false?'checked':'')+'> Search bar</label><label><input name="show_categories" type="checkbox" '+(settings.show_categories!==false?'checked':'')+'> Category navigation</label><label><input name="show_announcement" type="checkbox" '+(settings.show_announcement===true?'checked':'')+'> Show announcement bar</label></div><div class="masterActions"><button class="primary">Save Header</button></div></form>';wireAssetUploads($('headerForm'));$('headerForm').onsubmit=async e=>{e.preventDefault();const f=e.currentTarget,d=Object.fromEntries(new FormData(f));d.show_search=f.elements.show_search.checked;d.show_categories=f.elements.show_categories.checked;d.show_announcement=f.elements.show_announcement.checked;await saveSettings(d);render()};return}
    if(section==='home'){p.innerHTML='<form id="homeForm" class="card builderForm"><div class="builderSectionHead"><div><span class="capEyebrow">HOME PAGE</span><h3>Hero & Sections</h3></div></div><div class="formGrid"><label>Hero Title<input name="hero_title" value="'+esc(settings.hero_title||settings.store_name||model.company.name)+'"></label><label>Hero Media Type<select name="hero_media_type"><option value="image" '+((settings.hero_media_type||'image')==='image'?'selected':'')+'>Image</option><option value="video" '+(settings.hero_media_type==='video'?'selected':'')+'>Video</option></select></label><label class="span2">Hero Subtitle<input name="hero_subtitle" value="'+esc(settings.hero_subtitle||'')+'"></label><label class="span2">Hero Image / Video URL<input name="hero_media_url" value="'+esc(settings.hero_media_url||'')+'"><span class="assetUploadRow"><input class="storeAssetInput" data-target="input[name=hero_media_url]" type="file" accept="image/*,video/*"><small class="assetStatus">Upload & compress</small></span></label><label>About Heading<input name="about_title" value="'+esc(settings.about_title||'About Us')+'"></label><label>Footer Text<input name="footer_text" value="'+esc(settings.footer_text||'')+'"></label></div><label>About Text<textarea name="about_text">'+esc(settings.about_text||'')+'</textarea></label><div class="builderToggleGrid"><label><input name="show_featured" type="checkbox" '+(settings.show_featured!==false?'checked':'')+'> Featured products</label><label><input name="show_media" type="checkbox" '+(settings.show_media!==false?'checked':'')+'> Media / offers section</label></div><div class="masterActions"><button class="primary">Save Home Page</button></div></form>';wireAssetUploads($('homeForm'));$('homeForm').onsubmit=async e=>{e.preventDefault();const f=e.currentTarget,d=Object.fromEntries(new FormData(f));d.show_featured=f.elements.show_featured.checked;d.show_media=f.elements.show_media.checked;await saveSettings(d);render()};return}
    if(section==='media'){p.innerHTML='<div class="builderTwoCol"><div class="card">'+mediaTable(media)+'</div><form id="mediaForm" class="card builderForm"><h3>'+(editingMedia?'Edit Media':'Add Media')+'</h3><div class="formGrid"><label>Type<select name="media_type"><option value="image" '+(editingMedia?.media_type!=='video'?'selected':'')+'>Image</option><option value="video" '+(editingMedia?.media_type==='video'?'selected':'')+'>Video</option></select></label><label>Placement<select name="placement">'+['home','hero','offer','gallery'].map(x=>'<option value="'+x+'" '+((editingMedia?.placement||'home')===x?'selected':'')+'>'+x+'</option>').join('')+'</select></label><label>Title<input name="title" value="'+esc(editingMedia?.title||'')+'"></label><label>Sort Order<input name="sort_order" type="number" min="0" value="'+esc(editingMedia?.sort_order||0)+'"></label><label class="span2">Media URL<input name="media_url" required value="'+esc(editingMedia?.media_url||'')+'"><span class="assetUploadRow"><input class="storeAssetInput" data-target="input[name=media_url]" type="file" accept="image/*,video/*"><small class="assetStatus">Upload & compress</small></span></label><label class="span2">Optional Link URL<input name="link_url" value="'+esc(editingMedia?.link_url||'')+'"></label></div><label><input name="active" type="checkbox" '+(editingMedia?.active!==false?'checked':'')+'> Active</label><div class="masterActions">'+(editingMedia?'<button type="button" id="cancelMediaEdit" class="secondary">Cancel</button>':'')+'<button class="primary">Save Media</button></div></form></div>';wireMedia();return}
    if(section==='categories'){p.innerHTML='<div class="builderTwoCol"><div class="card">'+categoryTable(categories)+'</div><form id="categoryForm" class="card builderForm"><h3>'+(editingCategory?'Edit Category':'Add Category')+'</h3><div class="formGrid"><label>Category Name<input name="category_name" required value="'+esc(editingCategory?.category_name||'')+'"></label><label>Slug<input name="slug" value="'+esc(editingCategory?.slug||'')+'"></label><label>Image URL<input name="image_url" value="'+esc(editingCategory?.image_url||'')+'"><span class="assetUploadRow"><input class="storeAssetInput" data-target="input[name=image_url]" type="file" accept="image/*"><small class="assetStatus">Upload image</small></span></label><label>Sort Order<input name="sort_order" type="number" min="0" value="'+esc(editingCategory?.sort_order||0)+'"></label></div><label><input name="active" type="checkbox" '+(editingCategory?.active!==false?'checked':'')+'> Active</label><div class="masterActions">'+(editingCategory?'<button type="button" id="cancelCategoryEdit" class="secondary">Cancel</button>':'')+'<button class="primary">Save Category</button></div></form></div>';wireCategories();return}
    if(section==='products'){const imgs=Array.isArray(editingProduct?.images)?editingProduct.images:[];p.innerHTML='<div class="builderTwoCol productsBuilder"><div class="card">'+productTable(products)+'</div><form id="productForm" class="card builderForm"><h3>'+(editingProduct?'Edit Product':'Add Store Product')+'</h3><div class="formGrid"><label>SKU<input name="sku" required value="'+esc(editingProduct?.sku||'')+'"></label><label>Product Name<input name="product_name" required value="'+esc(editingProduct?.product_name||'')+'"></label><label>Category<select name="category_id"><option value="">No Category</option>'+categories.filter(x=>x.active).map(x=>'<option value="'+x.id+'" '+(Number(editingProduct?.category_id)===Number(x.id)?'selected':'')+'>'+esc(x.category_name)+'</option>').join('')+'</select></label><label>Sort Order<input name="sort_order" type="number" min="0" value="'+esc(editingProduct?.sort_order||0)+'"></label><label>Sale Price<input name="price" type="number" min="0" step="0.01" required value="'+esc(editingProduct?.price||0)+'"></label><label>Compare Price<input name="compare_at_price" type="number" min="0" step="0.01" value="'+esc(editingProduct?.compare_at_price||'')+'"></label><label>Store Stock<input name="stock_qty" type="number" min="0" step="0.001" required value="'+esc(editingProduct?.stock_qty||0)+'"></label><label>Main Image URL<input name="image_url" value="'+esc(editingProduct?.image_url||imgs[0]||'')+'"><span class="assetUploadRow"><input class="storeAssetInput" data-target="input[name=image_url]" type="file" accept="image/*"><small class="assetStatus">Upload</small></span></label><label>Image 2 URL<input name="image_2" value="'+esc(imgs[1]||'')+'"><span class="assetUploadRow"><input class="storeAssetInput" data-target="input[name=image_2]" type="file" accept="image/*"><small class="assetStatus">Upload</small></span></label><label>Image 3 URL<input name="image_3" value="'+esc(imgs[2]||'')+'"><span class="assetUploadRow"><input class="storeAssetInput" data-target="input[name=image_3]" type="file" accept="image/*"><small class="assetStatus">Upload</small></span></label></div><label>Description<textarea name="description">'+esc(editingProduct?.description||'')+'</textarea></label><label><input name="featured" type="checkbox" '+(editingProduct?.featured?'checked':'')+'> Featured Product</label><div class="masterActions">'+(editingProduct?'<button type="button" id="cancelProductEdit" class="secondary">Cancel</button>':'')+'<button class="primary">Save Product</button></div></form></div>';wireProducts();return}
    if(section==='orders'){p.innerHTML='<div class="card"><div class="builderSectionHead"><div><h3>Orders</h3><p>Website orders.</p></div><button id="ecomSalesReport" class="secondary">Sales Statement</button></div>'+orderTable(orders)+'</div>';wireOrders();$('ecomSalesReport').onclick=()=>openEcommerceSalesReport().catch(e=>alert(e.message));return}
    if(section==='contact'){p.innerHTML='<form id="contactForm" class="card builderForm"><div class="builderSectionHead"><div><span class="capEyebrow">CONTACT & DELIVERY</span><h3>Customer Information</h3></div></div><div class="formGrid"><label>Phone<input name="contact_phone" value="'+esc(settings.contact_phone||'')+'"></label><label>WhatsApp<input name="whatsapp_number" value="'+esc(settings.whatsapp_number||'')+'"></label><label>Delivery Charge<input name="delivery_charge" type="number" min="0" step="0.01" value="'+esc(settings.delivery_charge||0)+'"></label></div><label>Store Address<textarea name="address">'+esc(settings.address||'')+'</textarea></label><div class="masterActions"><button class="primary">Save Contact</button></div></form>';$('contactForm').onsubmit=async e=>{e.preventDefault();await saveSettings(Object.fromEntries(new FormData(e.currentTarget)));render()};return}
    if(section==='seo'){p.innerHTML='<form id="seoForm" class="card builderForm"><div class="builderSectionHead"><div><span class="capEyebrow">DISCOVERY & SOCIAL</span><h3>SEO & Social Links</h3></div></div><div class="formGrid"><label>SEO Page Title<input name="seo_title" value="'+esc(settings.seo_title||'')+'"></label><label>Instagram URL<input name="instagram_url" value="'+esc(settings.instagram_url||'')+'"></label><label>Facebook URL<input name="facebook_url" value="'+esc(settings.facebook_url||'')+'"></label><label>TikTok URL<input name="tiktok_url" value="'+esc(settings.tiktok_url||'')+'"></label></div><label>SEO Description<textarea name="seo_description">'+esc(settings.seo_description||'')+'</textarea></label><div class="masterActions"><button class="primary">Save SEO</button></div></form>';$('seoForm').onsubmit=async e=>{e.preventDefault();await saveSettings(Object.fromEntries(new FormData(e.currentTarget)));render()};return}
    if(section==='publish'){p.innerHTML='<div class="card publishCard"><span class="capEyebrow">PREVIEW & PUBLISH</span><h3>'+esc(settings.store_name||model.company.name)+'</h3><p>Header par sirf company branding hogi. Bizora sirf footer ki slim “Powered by Bizora” strip mein hoga.</p><div class="publishStatus '+(settings.published!==false?'live':'draft')+'"><b>'+(settings.published!==false?'Website Published':'Website Draft')+'</b><span>'+esc(storeUrl)+'</span></div><div class="builderToggleGrid"><label><input id="storeActiveToggle" type="checkbox" '+(settings.active!==false?'checked':'')+'> Store Active</label><label><input id="storePublishedToggle" type="checkbox" '+(settings.published!==false?'checked':'')+'> Publicly Published</label></div><div class="publishActions"><a class="secondary" href="'+storeUrl+'" target="_blank" rel="noopener">Preview Website</a><button id="savePublish" class="primary">Save Publish Status</button></div></div>';$('savePublish').onclick=async()=>{await saveSettings({active:$('storeActiveToggle').checked,published:$('storePublishedToggle').checked});render()};return}
  };

  const render=()=>{
    $('workspaceBody').innerHTML='<div class="storeBuilderHead"><div><span class="capEyebrow">NO-CODE WEBSITE BUILDER</span><h2>'+esc(settings.store_name||model.company.name)+'</h2><p>Company apni website khud design, preview aur publish kar sakti hai.</p></div><div class="storeBuilderHeadActions"><a class="secondary" href="'+storeUrl+'" target="_blank" rel="noopener">Open Website</a><span class="pill '+(settings.published!==false?'active':'inactive')+'">'+(settings.published!==false?'Published':'Draft')+'</span></div></div><div class="storeBuilderShell"><aside class="storeBuilderNav">'+navItems.map(([id,label])=>'<button type="button" data-section="'+id+'" class="'+(section===id?'active':'')+'">'+esc(label)+'<span>›</span></button>').join('')+'</aside><section id="storeBuilderPanel" class="storeBuilderPanel"></section></div>';
    document.querySelectorAll('.storeBuilderNav button').forEach(b=>b.onclick=()=>{section=b.dataset.section;editingProduct=editingCategory=editingMedia=null;render()});
    renderPanel();
  };
  render();
}
