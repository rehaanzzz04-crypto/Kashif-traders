...
const defs={
  ...
  'client-bills':{title:'Client Bills',action:'client_invoices',create:'create_client_invoice',cols:[['invoice_number','Invoice'],['business_name','Client'],['invoice_date','Date'],['due_date','Due'],['amount','Amount'],['status','Status']],fields:[['client_id','Client','client'],['invoice_number','Invoice Number','text'],['invoice_date','Invoice Date','date'],['due_date','Due Date','date'],['amount','Amount','number'],['notes','Notes','textarea']]},
  'client-payments':{title:'Client Payments',action:'client_receipts',create:'create_client_receipt',cols:[['receipt_date','Date'],['business_name','Client'],['amount','Amount'],['payment_method','Method'],['reference_number','Reference']],fields:[['client_id','Client','client'],['receipt_date','Receipt Date','date'],['amount','Amount','number'],['payment_method','Method','select:CASH,BANK,ONLINE,CHEQUE,EASYPAISA,JAZZCASH'],['reference_number','Reference','text'],['notes','Notes','textarea']]},
...
function dashboard(){
  ...
  const s=model.stats||{};$('workspaceBody').innerHTML='<div class="stats">'+[['Users',s.users],['Suppliers',s.suppliers],['Clients',s.clients],['Products',s.products],['Warehouses',s.warehouses],['Supplier Payable',money(s.supplier_payable)],['Client Receivable',money(s.client_receivable)]].map(([k,v])=>'<div class="stat"><small>'+k+'</small><b>'+esc(v??0)+'</b></div>').join('')+'</div><div class="card subscription"><h2>Subscription</h2><div class="subgrid"><div><small>Plan</small><b>'+esc(model.subscription.plan_name||'—')+'</b></div><div><small>Expires</small><b>'+date(model.subscription.expires_on)+'</b></div><div><small>Access</small><b>'+esc(model.subscription.access_mode)+'</b></div><div><small>Role</small><b>'+esc(model.user.role)+'</b></div><div><small>User Limit</small><b>'+esc(model.limits.user_limit??'Unlimited')+'</b></div><div><small>Warehouse Limit</small><b>'+esc(model.limits.warehouse_limit??'Unlimited')+'</b></div></div></div>';
}
