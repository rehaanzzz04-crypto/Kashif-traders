...
async function openEcommerce(){
  ...
  const render=()=>{
    ...
    $('workspaceBody').innerHTML=
      '<div class="ecomHead"><div><span class="capEyebrow">SEPARATE STORE SYSTEM</span><h2>'+esc(settings.store_name||model.company.name)+'</h2><p>Store products, stock and orders ERP products/invoices se separate hain.</p></div><div class="ecomHeadActions"><a class="secondary ecomOpenStore" href="'+publicStoreUrl+'" target="_blank" rel="noopener">Open Store</a><span class="planBadge">'+esc(model.subscription.plan_name||'Plan')+'</span></div></div>'+
