...
(() => {
  ...
  async function video(file, progress) {
    ...
    if (file.size <= 750000) return file;
    if (!window.MediaRecorder || !HTMLCanvasElement.prototype.captureStream) {
      if (file.size < 3900000) return file;
      throw Error('Is phone par video compression supported nahi. 4 MB se choti video select karein.');
    }
    ...
    if (!mime) {URL.revokeObjectURL(source.src);if(file.size<3900000)return file;throw Error('Is phone par video compression supported nahi.');}
    const targetBits=Math.max(220000,Math.min(1100000,Math.floor((3.3*MB*8/duration)-64000))),chunks=[];
    const recorder=new MediaRecorder(stream,{mimeType:mime,videoBitsPerSecond:targetBits,audioBitsPerSecond:64000});recorder.ondataavailable=e=>{if(e.data.size)chunks.push(e.data);};
    ...
    if (!blob.size || blob.size >= 3900000) throw Error('Video 4 MB se neeche compress nahi hui. Choti video select karein.');
