...
let model={stats:{},companies:[],plans:[],audit_prices:[],audit_requests:[]};
...
function render(){
  const s=model.stats||{};
  $('stats').innerHTML=[['Companies',s.companies??0,'▦'],['Active Companies',s.active_companies??0,'▥'],['Active Subscriptions',s.active_subscriptions??0,'◉'],['Pending Audits',s.pending_audits??0,'✓']].map(([k,v,i])=>'<div class="stat" data-icon="'+i+'"><small>'+k+'</small><b>'+esc(v)+'</b></div>').join('');
