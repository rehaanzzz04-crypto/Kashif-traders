...
(()=>{
 ...
 async function scan(file,form,box){
  ...
  try{
   ...
   box.textContent='OCR complete — details verify karke Save karein.';note('OCR complete');return true;
  }catch(e){box.textContent='OCR unavailable — form manually fill kiya ja sakta hai.';note(e.message,true);return false;}
 }
 async function upload(file,form,box,quiet=false){
  if(!file)return false;
  if(file.size>4000000){note('Document 4 MB se chhota rakhein.',true);return false;}
  if(!quiet)box.textContent='Uploading document…';
  try{
   ...
   if(!quiet){box.textContent='Document attached.';note('Document attached');}return true;
  }catch(e){if(!quiet){box.textContent='Upload unavailable — record save ab bhi use ho sakta hai.';note(e.message,true);}return false;}
 }
 function enhance(){
  ...
  const panel=document.createElement('div');panel.className='field full';panel.innerHTML='<label>Scan / Attach Document</label><input id="ktCameraFile" type="file" accept="image/*" capture="environment" style="display:none"><input id="ktAttachFile" type="file" accept="image/*,application/pdf" style="display:none"><div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:4px"><button type="button" class="btn light" id="ktScanBtn">📷 Scan with OCR</button><button type="button" class="btn alt" id="ktUploadBtn">Attach File</button></div><small id="ktScanStatus" style="display:block;margin-top:7px">Scan with OCR camera kholega. Attach File gallery ya PDF ke liye hai.</small>';
  const grid=$(form,'.grid');grid?.appendChild(panel);
  const camera=$(panel,'#ktCameraFile'),attach=$(panel,'#ktAttachFile'),status=$(panel,'#ktScanStatus'),scanBtn=$(panel,'#ktScanBtn'),uploadBtn=$(panel,'#ktUploadBtn');
  scanBtn.onclick=()=>camera.click();
  camera.onchange=async()=>{const f=camera.files?.[0];if(!f)return;scanBtn.disabled=true;uploadBtn.disabled=true;try{const ok=await scan(f,form,status);status.textContent=ok?'OCR complete — photo attach ho rahi hai…':'OCR complete nahi hua — photo attach ki ja rahi hai…';const attached=await upload(f,form,status,true);status.textContent=attached?(ok?'OCR complete + photo attached. Details verify karke Save karein.':'Photo attached. Details manually verify karke Save karein.'):(ok?'OCR complete. Photo attach nahi hui; record phir bhi save ho sakta hai.':'OCR/upload unavailable — form manually fill kiya ja sakta hai.');}finally{scanBtn.disabled=false;uploadBtn.disabled=false;camera.value='';}};
  uploadBtn.onclick=()=>attach.click();
  attach.onchange=async()=>{const f=attach.files?.[0];if(!f)return;scanBtn.disabled=true;uploadBtn.disabled=true;try{await upload(f,form,status);}finally{scanBtn.disabled=false;uploadBtn.disabled=false;attach.value='';}};
 }
