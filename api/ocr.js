...
export default async function handler(req,res){
 ...
 try{
  ...
  const key=process.env.OPENAI_API_KEY;if(!key)return res.status(503).json({error:'OCR service is not configured yet'});
  ...
  const r=await fetch('https://api.openai.com/v1/responses',{method:'POST',headers:{Authorization:'Bearer '+key,'Content-Type':'application/json'},body:JSON.stringify({model:'gpt-5-mini',input:[{role:'user',content:[{type:'input_text',text:prompt},{type:'input_image',image_url:dataUrl}]}],max_output_tokens:300})});
