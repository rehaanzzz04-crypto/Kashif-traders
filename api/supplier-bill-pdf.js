...
export default async function handler(req, res) {
  ...
  try {
    ...
    const sql = neon(url);
    if (!await getSessionUser(req, sql)) return res.status(401).json({ error: 'Authentication required' });
    ...
    const bill = bills[0], pdf = await PDFDocument.create(), page = pdf.addPage([595, 842]);
    ...
    page.drawText('KASHIF TRADERS', { x:105, y:798, size:19, font:bold, color:white });
    ...
    let y=610, itemTotal=0;
    page.drawRectangle({ x:34, y:y-6, width:527, height:24, color:green });
    [['Product',38],['Qty',330],['Rate',390],['Total',480]].forEach(([t,x]) => page.drawText(t,{x,y,size:10,font:bold,color:white}));
    y-=30;
    for (const item of items) {
      const qty=Number(item.quantity||0), rate=Number(item.unit_price||0), total=qty*rate; itemTotal+=total;
      page.drawRectangle({x:34,y:y-5,width:527,height:22,color:y%44===0?soft:white});
      ...
      page.drawText(money(total),{x:480,y,size:9,font:regular,color:ink}); y-=22;
      if (y<110) break;
    }
    ...
    return res.status(200).send(Buffer.from(await pdf.save()));
