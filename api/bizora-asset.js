...
export default async function handler(req,res){
  ...
  try{
    if(req.method==='GET'){
      ...
      const rows=await sql`SELECT mime_type,content FROM ecommerce_assets WHERE id=${id} LIMIT 1`;
      const row=rows[0];
      if(!row)return res.status(404).end('Asset not found');
      res.setHeader('Content-Type',row.mime_type);
      res.setHeader('Cache-Control','public, max-age=31536000, immutable');
      return res.status(200).send(Buffer.from(row.content));
    }
    ...
    const rows=await sql`INSERT INTO ecommerce_assets(company_id,file_name,mime_type,byte_size,content,created_by_user_id)
      VALUES(${u.company_id},${name||null},${mime},${buf.length},${buf},${u.id})
