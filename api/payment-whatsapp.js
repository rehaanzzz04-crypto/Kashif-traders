...
async function optimizeBillImage(src){
 try{
  const base=sharp(src.buf,{failOn:'none'}).rotate();
  const meta=await base.metadata();
  const w=Number(meta.width||0),h=Number(meta.height||0);
  if(!w||!h)throw Error('Image dimensions unavailable');
  /* Supplier uploads are commonly phone screenshots/photos. Trim uniform dark/light
     outer areas first; then remove a conservative portion of a tall screenshot's
     phone chrome while preserving the complete paper/document. */
  let img=base.trim({background:'#111111',threshold:12});
  const ratio=h/w;
  if(ratio>1.75){const top=Math.round(h*.075),bottom=Math.round(h*.065),left=Math.round(w*.015),right=Math.round(w*.015);img=sharp(src.buf,{failOn:'none'}).rotate().extract({left,top,width:Math.max(1,w-left-right),height:Math.max(1,h-top-bottom)}).trim({background:'#111111',threshold:14})}
 ...
 }catch(e){console.warn('Supplier bill image optimization skipped',e?.message||e);return src}
}
