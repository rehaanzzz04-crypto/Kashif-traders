...
async function optimizeBillImage(src){
 ...
 }catch(e){console.warn('Supplier document smart crop skipped',e?.message||e);return src}
}
...
async function generateSupplierPaymentPdf(sql,recordId){
 const rows=await sql`SELECT p.*,s.business_name,s.opening_balance FROM supplier_payments p JOIN suppliers s ON s.id=p.supplier_id WHERE p.id=${recordId}`;
 const r=rows[0];if(!r)throw Error('Supplier payment not found');
 const billTotal=Number((await sql`SELECT COALESCE(SUM(amount),0) total FROM supplier_invoices WHERE supplier_id=${r.supplier_id}`)[0]?.total||0),payTotal=Number((await sql`SELECT COALESCE(SUM(amount),0) total FROM supplier_payments WHERE supplier_id=${r.supplier_id}`)[0]?.total||0),balance=Number(r.opening_balance||0)+billTotal-payTotal;
 const pdf=await PDFDocument.create(),font=await pdf.embedFont(StandardFonts.Helvetica),bold=await pdf.embedFont(StandardFonts.HelveticaBold),p=pdf.addPage([595,842]);brand(p,font,bold,'SUPPLIER PAYMENT');
 p.drawText('SUPPLIER',{x:34,y:687,size:8,font:bold,color:muted});p.drawText(fit(r.business_name,bold,15,330),{x:34,y:663,size:15,font:bold,color:ink});p.drawRectangle({x:385,y:630,width:176,height:88,color:soft});
 const no=clean(r.entry_number)||`SP-${r.id}`;p.drawText(`ERP No: ${fit(no,font,8,120)}`,{x:395,y:697,size:8,font:bold,color:ink});p.drawText(`Payment Date: ${dateLabel(r.payment_date)}`,{x:395,y:680,size:8,font,color:ink});p.drawText(`Method: ${fit(r.payment_method,font,8,90)||'-'}`,{x:395,y:663,size:8,font,color:ink});p.drawText(`Bank: ${fit(r.bank,font,8,105)||'-'}`,{x:395,y:646,size:8,font,color:ink});p.drawText(`Reference: ${fit(r.reference_number,font,8,95)||'-'}`,{x:395,y:629,size:8,font,color:ink});
 p.drawRectangle({x:34,y:548,width:527,height:62,color:green});p.drawText('PAYMENT AMOUNT',{x:50,y:587,size:9,font:bold,color:white});p.drawText(money(r.amount),{x:50,y:559,size:22,font:bold,color:white});p.drawText('ACCOUNT POSITION AFTER THIS PAYMENT',{x:34,y:513,size:11,font:bold,color:green});
 let y=467;[['Opening Balance',Number(r.opening_balance||0)],['Total Supplier Bills',billTotal],['Total Supplier Payments',payTotal],['Current Outstanding',balance]].forEach((a,i)=>{p.drawRectangle({x:34,y,width:527,height:38,color:i===3?rgb(.95,.89,.70):white,borderColor:rgb(.84,.84,.80),borderWidth:.7});p.drawText(a[0],{x:50,y:y+14,size:9,font:i===3?bold:font,color:ink});const m=money(a[1]),f=i===3?bold:font,sz=i===3?11:9,w=f.widthOfTextAtSize(m,sz);p.drawText(m,{x:545-w,y:y+14,size:sz,font:f,color:i===3?green:ink});y-=44});
 if(clean(r.notes)){p.drawText('Notes',{x:34,y:269,size:9,font:bold,color:muted});p.drawText(fit(r.notes,font,9,510),{x:34,y:251,size:9,font,color:ink})}p.drawLine({start:{x:34,y:70},end:{x:561,y:70},thickness:1,color:gold});p.drawText('Quality Products  |  Reliable Supply  |  Growing Together',{x:34,y:48,size:8,font:bold,color:green});
 const src=await sourceImage(r.attachment_url);p.drawText(src?'Page 1 of 2':'Page 1 of 1',{x:500,y:48,size:8,font:bold,color:green});if(src){const p2=pdf.addPage([595,842]);brand(p2,font,bold,'ORIGINAL PAYMENT SLIP');p2.drawText(`${fit(r.business_name,font,8,280)}  •  ${clean(r.reference_number)||no}  •  ${dateLabel(r.payment_date)}`,{x:34,y:710,size:8,font,color:muted});const prepared=await optimizeBillImage(src);let img=null;try{img=prepared.type.includes('png')?await pdf.embedPng(prepared.buf):await pdf.embedJpg(prepared.buf)}catch{}if(img){const maxW=527,maxH=628,scale=Math.min(maxW/img.width,maxH/img.height),w=img.width*scale,h=img.height*scale,x=34+(maxW-w)/2,y2=68+(maxH-h)/2;p2.drawRectangle({x:33,y:67,width:529,height:630,color:white,borderColor:rgb(.84,.84,.80),borderWidth:1});p2.drawImage(img,{x,y:y2,width:w,height:h})}p2.drawText('Smart-cropped payment slip from Kashif Traders ERP',{x:34,y:45,size:8,font,color:muted});p2.drawText('Page 2 of 2',{x:500,y:45,size:8,font:bold,color:green})}
 const buffer=Buffer.from(await pdf.save()),filename=((clean(r.reference_number)||`supplier-payment-${r.id}`).replace(/[^a-z0-9_-]+/gi,'-')||`supplier-payment-${r.id}`)+'.pdf';return{buffer,filename};
}
export default async function handler(req,res){
 ...
 try{
  ...
  if(type==='supplier_payment_pdf'){const doc=await generateSupplierPaymentPdf(sql,recordId);res.setHeader('Content-Type','application/pdf');res.setHeader('Content-Disposition',`attachment; filename="${doc.filename}"`);res.setHeader('Cache-Control','no-store');return res.status(200).send(doc.buffer);}
