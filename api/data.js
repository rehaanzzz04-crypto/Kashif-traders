...
const resourceView={suppliers:'suppliers',supplier_invoices:'supplier-bills',supplier_payments:'supplier-payments',clients:'clients',client_invoices:'client-sales',client_receipts:'client-receipts',documents:'documents'};

function db(){const url=process.env.DATABASE_URL;if(!url) throw new Error('DATABASE_URL_NOT_CONFIGURED');return neon(url);}
...
function cleanText(v){if(v===undefined||v===null)return null;const s=String(v).trim();return s===''?null:s;}
function cleanAmount(v){if(v===undefined||v===null||v==='')return null;const n=Number(v);return Number.isFinite(n)?n:null;}

async function list(sql,resource,id){
  if(resource==='suppliers'){if(id)return sql`SELECT * FROM suppliers WHERE id=${id}`;return sql`SELECT * FROM suppliers ORDER BY business_name ASC,id ASC`;}
  if(resource==='clients'){if(id)return sql`SELECT * FROM clients WHERE id=${id}`;return sql`SELECT * FROM clients ORDER BY business_name ASC,id ASC`;}
  if(resource==='supplier_invoices'){if(id)return sql`SELECT i.*,s.business_name FROM supplier_invoices i JOIN suppliers s ON s.id=i.supplier_id WHERE i.id=${id}`;return sql`SELECT i.*,s.business_name FROM supplier_invoices i JOIN suppliers s ON s.id=i.supplier_id ORDER BY i.invoice_date DESC,i.id DESC`;}
  if(resource==='supplier_payments'){if(id)return sql`SELECT p.*,s.business_name FROM supplier_payments p JOIN suppliers s ON s.id=p.supplier_id WHERE p.id=${id}`;return sql`SELECT p.*,s.business_name FROM supplier_payments p JOIN suppliers s ON s.id=p.supplier_id ORDER BY p.payment_date DESC,p.id DESC`;}
  if(resource==='client_invoices'){if(id)return sql`SELECT i.*,c.business_name FROM client_invoices i JOIN clients c ON c.id=i.client_id WHERE i.id=${id}`;return sql`SELECT i.*,c.business_name FROM client_invoices i JOIN clients c ON c.id=i.client_id ORDER BY i.invoice_date DESC,i.id DESC`;}
  if(resource==='client_receipts'){if(id)return sql`SELECT r.*,c.business_name FROM client_receipts r JOIN clients c ON c.id=r.client_id WHERE r.id=${id}`;return sql`SELECT r.*,c.business_name FROM client_receipts r JOIN clients c ON c.id=r.client_id ORDER BY r.receipt_date DESC,r.id DESC`;}
  if(resource==='documents'){if(id)return sql`SELECT * FROM documents WHERE id=${id}`;return sql`SELECT * FROM documents ORDER BY created_at DESC,id DESC`;}
  return [];
}
async function create(sql,resource,b){
  if(resource==='suppliers')return sql`INSERT INTO suppliers(business_name,contact_person,mobile_number,whatsapp_number,address,opening_balance,notes,status) VALUES(${cleanText(b.business_name)},${cleanText(b.contact_person)},${cleanText(b.mobile_number)},${cleanText(b.whatsapp_number)},${cleanText(b.address)},${cleanAmount(b.opening_balance)??0},${cleanText(b.notes)},${cleanText(b.status)??'active'}) RETURNING *`;
  if(resource==='clients')return sql`INSERT INTO clients(business_name,contact_person,mobile_number,whatsapp_number,address,credit_limit,opening_balance,notes,status) VALUES(${cleanText(b.business_name)},${cleanText(b.contact_person)},${cleanText(b.mobile_number)},${cleanText(b.whatsapp_number)},${cleanText(b.address)},${cleanAmount(b.credit_limit)},${cleanAmount(b.opening_balance)??0},${cleanText(b.notes)},${cleanText(b.status)??'active'}) RETURNING *`;
...
export default async function handler(req,res){
  ...
  try{
    ...
    if(req.method==='PATCH'){if(!id)return res.status(400).json({error:'Valid id is required'});const records=await patch(sql,resource,id,bodyOf(req));return res.status(200).json({record:records[0]||null});}
    if(req.method==='DELETE'){if(!id)return res.status(400).json({error:'Valid id is required'});const records=await remove(sql,resource,id);return res.status(200).json({deleted:Boolean(records[0])});}
    return res.status(405).json({error:'Method not allowed'});
  }catch(e){console.error('Kashif Traders API error',e);const msg=e?.message==='DATABASE_URL_NOT_CONFIGURED'?'DATABASE_URL is not configured':(e?.code==='23505'?'Duplicate record':e?.code==='23503'?'This record is linked to other data':'Database request failed');return res.status(e?.message==='DATABASE_URL_NOT_CONFIGURED'?503:500).json({error:msg});}
}
