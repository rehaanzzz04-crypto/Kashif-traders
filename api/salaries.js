...
export default async function handler(req,res){
 try{
  const auth=await requireUser(req,res);if(!auth)return;const{sql,user}=auth;await ensureEntryNumbers(sql);
  ...
  if(req.method==='GET'){
   ...
   return res.status(200).json({records:numbered});
  }
  if(req.method==='POST'){
   ...
   return res.status(201).json({record:{...rows[0],entry_number:null},pending_approval:true,message:'Sent to Admin for approval — final number is assigned only after approval'});
  }
  if(req.method==='PATCH'){
