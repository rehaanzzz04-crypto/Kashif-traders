...
const monthStart=v=>/^\d{4}-\d{2}(?:-\d{2})?$/.test(String(v||''))?String(v).slice(0,7)+'-01':null;
...
export default async function handler(req,res){
 try{
  ...
  if(req.method==='POST'){
   ...
   const requestedMonth=monthStart(b.salary_month),nowMonth=(await sql`SELECT date_trunc('month',current_date)::date AS m`)[0]?.m;
   const salaryMonth=type==='monthly_salary'?requestedMonth:(requestedMonth||String(nowMonth).slice(0,10));
  ...
  if(req.method==='PATCH'){
   ...
   if(action==='approve'||action==='reject'){
    ...
    if(action==='approve'){
      const p=await profile(sql,cur.employee_id),fixed=Number(p?.monthly_salary||0),m=String(cur.salary_month||cur.requested_at).slice(0,7)+'-01',other=await committed(sql,cur.employee_id,m,cur.id);
