...
function extension(mime){const m=clean(mime).toLowerCase();if(m.includes('jpeg'))return'jpg';if(m.includes('png'))return'png';if(m.includes('webp'))return'webp';if(m.includes('gif'))return'gif';if(m.includes('pdf'))return'pdf';if(m.includes('svg'))return'svg';if(m.includes('mp4'))return'mp4';if(m.includes('webm'))return'webm';if(m.includes('quicktime'))return'mov';return'bin'}
...
export default async function handler(req,res){
 try{
  ...
  const chunks=[];let size=0;for await(const chunk of req){size+=chunk.length;if(size>4000000)return res.status(413).json({error:'Media is too large (max 4 MB)'});chunks.push(chunk)}
  const type=String(req.headers['content-type']||'application/octet-stream').split(';')[0];if(!['image/jpeg','image/png','image/webp','application/pdf','video/mp4','video/webm','video/quicktime'].includes(type))return res.status(415).json({error:'Only JPG, PNG, WebP, PDF, MP4, WebM or MOV is allowed'});
