...
export default async function handler(req,res){
 ...
 try{
  ...
  if(!process.env.BLOB_READ_WRITE_TOKEN)return res.status(503).json({error:'Document storage is not configured yet'});
  let put;try{({put}=await import('@vercel/blob'))}catch{return res.status(503).json({error:'Document storage package is not installed yet'})}
  ...
  const raw=String(req.query?.name||'document').replace(/[^a-zA-Z0-9._-]/g,'_').slice(-100),name=Date.now()+'-'+raw;
  const blob=await put('kashif-traders/documents/'+name,Buffer.concat(chunks),{access:'public',contentType:type,addRandomSuffix:true});
