import { db,bodyOf,cleanText,hashPin,newSalt,newToken,hashToken,setSessionCookie,clearSessionCookie,getSessionUser,ROLE_ACCESS } from './_auth.js';
...
export default async function handler(req,res){
  try{
    ...
    if(req.method==='GET'&&action==='status'){
      ...
      return res.status(200).json({hasEmployees:Number(count[0]?.count||0)>0,user:user?{...user,access:ROLE_ACCESS[user.designation]||[]}:null});
    }
    if(req.method==='POST'&&action==='bootstrap'){
      ...
      return res.status(201).json({user:{...rows[0],access:ROLE_ACCESS.admin}});
    }
    if(req.method==='POST'&&action==='login'){
      ...
      setSessionCookie(res,token);
      return res.status(200).json({user:{id:e.id,employee_code:e.employee_code,full_name:e.full_name,mobile_number:e.mobile_number,designation:e.designation,status:e.status,access:ROLE_ACCESS[e.designation]||[]}});
    }
    ...
    if(req.method==='GET'&&action==='me'){
      ...
      return res.status(200).json({user:{...user,access:ROLE_ACCESS[user.designation]||[]}});
    }
