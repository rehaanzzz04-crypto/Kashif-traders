...
async function persistMedia(media,mediaId){
 const m=String(media.dataUrl||'').match(/^data:(image\/(?:jpeg|png|webp));base64,(.+)$/i);if(!m)return null;const buf=Buffer.from(m[2],'base64');
 if(process.env.BLOB_READ_WRITE_TOKEN){try{const {put}=await import('@vercel/blob');const ext=m[1].includes('png')?'png':m[1].includes('webp')?'webp':'jpg';const blob=await put(`kashif-traders/whatsapp-invoices/${Date.now()}-${clean(mediaId)}.${ext}`,buf,{access:'public',contentType:m[1],addRandomSuffix:true});return blob.url}catch(e){console.error('WhatsApp invoice blob upload failed',e)}}
...
export default async function handler(req,res){
 ...
 try{
  ...
  const media=await downloadWhatsAppImage(mediaId),fields=await extractDocument(media.dataUrl,'supplier_invoice'),total=amount(fields.amount),attachmentUrl=await persistMedia(media,mediaId);
