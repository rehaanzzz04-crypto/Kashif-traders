...
(()=>{
  ...
  async function enhance(form){
    ...
    const wrap=document.createElement('div');wrap.className='field';wrap.innerHTML='<label>Supplier Invoice (Optional)</label><select name="supplier_invoice_id"><option value="">Auto-adjust oldest invoices</option></select><small data-invoice-balance style="display:block;margin-top:6px;color:#61736d">Leave blank for automatic FIFO adjustment</small>';
    ...
    async function load(){
      const sid=Number(supplier.value||0);invoice.innerHTML='<option value="">Loading…</option>';hint.textContent='Leave blank for automatic FIFO adjustment';
