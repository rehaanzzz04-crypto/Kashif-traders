...
(()=>{
  ...
  async function enhance(form){
    ...
    const wrap=document.createElement('div');wrap.className='field';wrap.innerHTML='<label>Supplier Invoice</label><select name="supplier_invoice_id" required><option value="">Select invoice</option></select><small data-invoice-balance style="display:block;margin-top:6px;color:#61736d"></small>';
    ...
    async function load(){
      const sid=Number(supplier.value||0);invoice.innerHTML='<option value="">Loading…</option>';hint.textContent='';
      if(!sid){invoice.innerHTML='<option value="">Select supplier first</option>';return}
      try{
        ...
        const opts=invoices.map(x=>{const paid=payments.filter(p=>Number(p.supplier_invoice_id)===Number(x.id)).reduce((s,p)=>s+Number(p.amount||0),0),remaining=Math.max(0,Number(x.amount||0)-paid);return {...x,paid,remaining}}).filter(x=>x.remaining>0.005);
        invoice.innerHTML='<option value="">Select invoice</option>'+opts.map(x=>'<option value="'+x.id+'" data-remaining="'+x.remaining+'">'+(x.invoice_number||x.entry_number||('Invoice '+x.id))+' — '+money(x.remaining)+' remaining</option>').join('');
        if(!opts.length)invoice.innerHTML='<option value="">No unpaid invoices</option>';
      }catch(e){invoice.innerHTML='<option value="">Unable to load invoices</option>';hint.textContent=e.message||'Invoice loading failed'}
    }
    function syncAmount(){const opt=invoice.selectedOptions?.[0],remaining=Number(opt?.dataset?.remaining||0);if(remaining>0){amount.value=String(remaining);amount.max=String(remaining);hint.textContent='Outstanding: '+money(remaining)}else{amount.removeAttribute('max');hint.textContent=''}}
  ...
  document.addEventListener('submit',async e=>{
    ...
    if(!invoiceId)return window.toast?.('Select supplier invoice',true);
    if(!(amt>0))return window.toast?.('Valid payment amount required',true);
    if(remaining>0&&amt>remaining+0.005)return window.toast?.('Payment exceeds invoice outstanding balance',true);
    const save=form.querySelector('[type="submit"]'),old=save?.textContent;if(save){save.disabled=true;save.textContent='Saving…'}
    try{
      const r=await fetch('/api/supplier-payment-invoice',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(b)}),j=await r.json().catch(()=>({}));if(!r.ok)throw new Error(j.error||'Supplier payment failed');
      document.getElementById('modal')?.remove();window.toast?.('Payment saved — invoice '+j.invoice.status);
